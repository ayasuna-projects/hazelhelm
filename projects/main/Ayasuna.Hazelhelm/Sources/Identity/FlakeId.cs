...
public static class FlakeId
{
    ...
    public static FlakeId<TEntity> FromString<TEntity>(string encodedId) where TEntity : notnull
    {
        ...
        return Create<TEntity>(timestamp, workerId, sequenceId);
    }
    ...
    /// </remarks>
    /// <param name="workerId">The worker id of the flake id, must be exactly 48 bits, typically the MAC address. </param>
    /// <exception cref="ArgumentOutOfRangeException">If the given <paramref name="workerId"/> is not exactly 48-bits long</exception>
    /// <typeparam name="TEntity">The entity type to create the id for</typeparam>
    /// <returns>The created flake id</returns>
    public static FlakeId<TEntity> Create<TEntity>(byte[] workerId) where TEntity : notnull
    {
        lock (Lock)
        {
            while (true)
    ...
    /// <param name="workerId">The worker id component, must be exactly 48 bits, typically the MAC address</param>
    ...
    /// <typeparam name="TEntity">The entity type to create the id for</typeparam>
    /// <returns>The created flake id</returns>
    public static FlakeId<TEntity> Create<TEntity>(DateTimeOffset timestamp, byte[] workerId, ushort sequenceId) where TEntity : notnull
    {
        ...
        {
            throw new ArgumentOutOfRangeException(nameof(workerId), $"The provided worker id was {workerId.Length * 8} bits long but must be exactly 48 bits long");
        }
        ...
        var bytes = new byte[timestampBytes.Length + workerId.Length + sequenceIdBytes.Length];

        Buffer.BlockCopy(sequenceIdBytes, 0, bytes, 0, sequenceIdBytes.Length);
        Buffer.BlockCopy(workerId, 0, bytes, sequenceIdBytes.Length, workerId.Length);
        Buffer.BlockCopy(timestampBytes, 0, bytes, sequenceIdBytes.Length + workerId.Length, timestampBytes.Length);
